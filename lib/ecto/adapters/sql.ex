defmodule Ecto.Adapters.SQL do
  ...
  def __before_compile__(conn, env) do
    config = Module.get_attribute(env.module, :config) |> normalize_config()

    if config[:pool] == Ecto.Adapters.SQL.Sandbox and config[:pool_size] == 1 do
    ...
    quote do
      ...
      @doc false
      def __pool__, do: {unquote(pool_name(env.module, config)), unquote(Macro.escape(config))}
